#FUNCTION getBestCluster (based on rsf)###########################################
# INPUT:
# dataset: dataframe
# nFeature: int, number of features
# rankby: "p.value" or "hr" or "cindex"

getBestCluster2 <- function(dataset, nFeature = 5, maxK = 15, optK = NULL,rankby = "hr", coef.threshold = NULL){
  ...
  for (i in 1:optK){
    ...
    features <- c(features, getBestRSF(cluster_df, nFeature = nFeature, ntree = 2000))
  }
